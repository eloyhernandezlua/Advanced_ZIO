      ...
      test("apply") {
        ...
        test("fromZIO") {
          ...
          for {
            ...
            stream = ZStream.fromZIO(Console.readLine)
            _      <- stream.foreach(value => ref.update(_ + value))
            v      <- ref.get
  ...
  def spec =
    suite("SimpleOperators") {
      ...
      test("runCollect") {
        ...
        for {
          values <- (stream.runCollect : UIO[Chunk[Int]])
        ...
        test("take") {
          for {
            values <- ZStream(1, 2, 3, 4).take(2).runCollect
        ...
        test("takeWhile") {
          for {
            values <- ZStream(1, 2, 3, 4).takeWhile(_ < 3).runCollect
          } yield assertTrue(values == Chunk(1, 2))
        } @@ ignore +
        /**
         * EXERCISE
        ...
        test("drop") {
          for {
            values <- ZStream(1, 2, 3, 4).drop(2).runCollect
        ...
        test("dropWhile") {
          for {
            values <- ZStream(1, 2, 3, 4).dropWhile(_ < 3).runCollect
          } yield assertTrue(values == Chunk(3, 4))
        } @@ ignore +
        /**
         * EXERCISE
        ...
        test("map") {
          for {
            values <- ZStream(1, 2, 3).map(_ * 2).runCollect
        ...
        test("filter") {
          for {
            values <- ZStream(1, 2, 3, 4, 5, 6).filter(_ % 2 == 0).runCollect
          } yield assertTrue(values == Chunk(2, 4, 6))
        } @@ ignore +
        /**
         * EXERCISE
        ...
        test("forever") {
          for {
            values <- ZStream(1).take(5).forever.runCollect
  ...
  def spec =
    suite("RunningStreams") {
      ...
      test("runHead") {
        ...
        //stream.foreach(Console.printLine(_))
        stream.runHead

        for {
          headOption <- ZIO.succeed(Option.empty[String])
        } yield assertTrue(headOption.contains("All work and no play makes Jack a dull boy"))
      } @@ ignore +
        /**
         * EXERCISE
         *
        ...
        test("runDrain") {
          ...
          for {
            ...
            _         <- (stream ++ ZStream.fromZIO(drained.set(true)).drain).runDrain
...
/*        test("foldZIO") {
          ...
          for {
            ...
            map <- questions.runFoldZIO(Map.empty)(ZSink.foldLeft(Map.empty[String, String]) {
                    case (map, question) =>
                      for {
                        _ <- Console.printLine(questions)
                        ans <- Console.readLine
                      } yield (map + (question -> ans))
                  })
...
object AdvancedConstructors extends ZIOSpecDefault {
  def spec =
    suite("AdvancedConstructors") {
       ...
       */
      test("unfold") {
        val fibs: ZStream[Any, Nothing, Int] = ZStream.unfold((0, 1)){
        ...
        test("repeatZIOOption") {
          ...
          val stream = ZStream.repeatZIOOption(readUntilJohn)
  ...
  def spec =
    suite("AdvancedOperators") {
      ...
      test("flatMap") {
        ...
        for {
          values <- stream.flatMap(a => ZStream(a,a,a)).runCollect
        ...
        test("mapZIO") {
          val questions =
            ZStream("What is your name?", "What is your age?").mapZIO(a => Console.readLine(a))
        ...
        test("mapAccum") {
          ...
          def aggregate(stream: Stream[Nothing, String]): Stream[Nothing, (String, Int)] =
            stream.mapAccum(Map.empty[String, Int]) {
              case (map, string) =>
                val updatedMap = map.updatedWith(string) {
                  case None        => Some(1)
                  case Some(value) => Some(value + 1)
                }
                updatedMap -> (string -> updatedMap(string))
            }
  ...
  def spec =
    suite("BasicError") {
      ...
      test("fail") {
        ...
        test("catchAll") {
          for {
            value <- ZStream.fail("Uh oh!").catchAll(error => ZStream(error)).runCollect
    ...
    suite("TemporalStreams") {
      ...
      test("fromSchedule") {
        ...
        test("repeatZIOWithSchedule") {
          ...
          val s = ZStream.repeatZIOWithSchedule(effect, schedule)
    ...
    suite("ChunkedStreams") {
      ...
      test("foreachChunk") {
        ...
        for {
          ...
          chunks     <- stream.chunks.foreach(chunk => chunkCount.update(_ + 1))
          v          <- chunkCount.get
...
object Graduation extends zio.ZIOAppDefault {
  def wordCount(file: String): IO[Throwable, Map[String, Int]] =
    (ZStream.fromFileName(file) >>>
      ...
      ZPipeline.map[String, String](_.split("\n").mkString(" ")) >>>
      ZPipeline.splitOn(" ") >>>
      ZPipeline.filter[String](_.forall(_.isLetter))
      ).runFold(Map.empty[String,Int]) { case (m, s) =>
      m.updatedWith(s)(_.fold(Some(1))(c => Some(c + 1)))
    }

  def run =
    getArgs.map(_.headOption).flatMap {
      case None => Console.printLine("You need to specify a file to word count")
      case Some(file) =>
        (for {
          map <- wordCount(file)
